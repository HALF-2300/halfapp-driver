/**
 * Free ride-flow Playwright ports (3024 frontend, 8011 backend) before E2E.
 */
import { execSync } from 'node:child_process'

const PORTS = [
  Number(process.env.PLAYWRIGHT_RIDE_FLOW_PORT || 3024),
  Number(process.env.PLAYWRIGHT_RIDE_FLOW_BACKEND_PORT || 8011),
]

const isWin = process.platform === 'win32'

function log(msg) {
  console.log(`[ride-flow-port] ${msg}`)
}
...
for (const port of PORTS) {
  ...
  for (const pid of pids) {
    ...
    try {
      ...
      console.error(`[ride-flow-port] Failed to stop PID ${pid}: ${err?.message || err}`)
      process.exit(1)
    }
  }
}

log('Ride-flow ports ready.')
